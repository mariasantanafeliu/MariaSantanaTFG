...
void ur3Control::cb_readForceTorque(const uma_fp_control::ftResponse& msg){
...
void ur3Control::cb_readSurface(const geometry_msgs::Point::ConstPtr& msg){
    /*desPose.x = msg->x;
    desPose.y = msg->y;
    desPose.z = msg->z;*/
    std::cout << "... cb_readSurface" << std::endl;
}
//#######################function##########################
...
void ur3Control::computeForceControl(double Kf){
    std::vector<double> compensatedSensorResponse = {0, 0, 0, 0, 0, 0};
    //auto R_E = moveit.transformationMatrix(); // Se calcula la matriz de transformación de Tool0
    Eigen::Matrix3d R_E = T_E.topLeftCorner<3,3>();
    Eigen::Vector3d pesoHerramienta(0,0,0); // Peso de la herramienta en coordenadas Base R
    // Pasar el peso a coordenadas relativas a E
    Eigen::Vector3d pesoHerramienta_ee(R_E.inverse()*pesoHerramienta);
    // Restar al peso total leído el peso por el sensor relativo de la herramienta
    Eigen::Vector3d respuestaSensor_F(HEXForce[0] - pesoHerramienta_ee[0], HEXForce[1] - pesoHerramienta_ee[1],HEXForce[2] - pesoHerramienta_ee[2]);
    // Transformar la fuerza relativa con la compensación de gravedad a dirección sobre los ejes de la base (absoluta)
    //Eigen::Vector3d respuestaSensorComp_F = R_E * respuestaSensor_F;
    compensatedSensorResponse[0] = -respuestaSensor_F[0];
    compensatedSensorResponse[1] = -respuestaSensor_F[1];
    compensatedSensorResponse[2] = -respuestaSensor_F[2];//ft.HEXForce[2];//
    if ((fabs(compensatedSensorResponse[0])>0.2) || (fabs(compensatedSensorResponse[1])>0.2)){
        fulcrumForces = true;
        Kforce->computeForceControl(compensatedSensorResponse, TCP[2], p_estimado,0, Kf, T_Fp, T_FpOG,Pdest, tr);
        T_Fp = Kforce->forceFulcrum.T_Fp;
        p_estimado = Kforce->forceFulcrum.p_anterior;
    } else{
        fulcrumForces = false;
    }
}
void ur3Control::computeRobotCinematic(bool flagFp){
    ...
    //std::cout << prefix_in << " T_TTP->" << std::endl;
...
void ur3Control::computeRobotVel(){
    ...
    //std::cout << prefix_in << " velVector= " << velVector[0] << ", "<< velVector[1] << ", "<< velVector[2] << ", "<< velVector[3] << ", "<< velVector[4] << ", "<< velVector[5] << std::endl;
...
int main(int argc, char **argv){
    ...
    ErrorPose error;
    forceControl Kforce;
    PIDController controlPosition(kp,ki,kd,0.008,6); //(double kp, double ki, double kd, double time_interval, int num_controllers)
    Cinematic T;
    selectTool tool;
