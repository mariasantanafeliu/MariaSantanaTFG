...
{       
	private:
                ...
                ErrorPose* error;
        public:
		tfg(ur_script * urScript, UMA_trans *umaTf, Init *initRobot, PIDController *composePID, selectTool *selectTool, fulcrum *fp, FTSensor* ftSensor, tf::TransformListener* tf_listener, ErrorPose *composeError, std::string t_prefix);
		~tfg();
                //variables
                        //init
                ...
                std_msgs::Float64MultiArray pose_, poseE_, array_vel, currentPolar_, desPolar_, deltaPolar_, fulcrum_position_, force_msg_, base_force_msg_, abdomen_force_effector_msg_, abdomen_force_msg_, maxwell_msg_;
                //time
                ros::Time start_time;
                ros::Time now;
                //force
		...
		double qX, qY, qZ, qW;
                ...
                Eigen::Vector3d computeTipForceControl(double Kf, std::vector<double> tipForces_base, Eigen::Vector3d v_cartesian);
